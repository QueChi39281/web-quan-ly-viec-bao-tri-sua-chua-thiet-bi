...
const safeMutationCall = async (
  requestFn,
  fallback = { success: true, updated: 0 },
) => {
  try {
...
export const normalizeNotificationForEmployee = (
  notification,
  ...
  const receiverList = Array.isArray(notification.receiver_info)
    ? notification.receiver_info
    : [];
  const matchedReceiver = receiverList.find((receiver) => {
    const current = String(receiver?.employee_id ?? "");
    return (
      current === targetEmployeeId ||
      Number(current) === Number(targetEmployeeId)
    );
  });
  ...
  const hasServerReadState = matchedReceiver
    ? matchedReceiver.is_read !== undefined ||
      matchedReceiver.read_at !== undefined
...
export const normalizeNotificationList = (
  ...
  return rawList.map((item) =>
    normalizeNotificationForEmployee(item, employeeId),
  );
};
  ...
  async (error) => {
    ...
    if (method === "get" && isMissingDataError(error)) {
      ...
      const looksLikeList =
        ...
        /(users|departments|devices|maintenance-requests|inventory|notifications|reports|transactions|items|stock|dashboard|employees)/.test(
          url,
        );
...
export const authApi = {
  ...
  activateAccount: (data) =>
    safeMutationCall(() => API.post("/auth/activate", data)),
  forgotPassword: (data) =>
    safeMutationCall(() =>
      API.post(
  ...
  resetPassword: (data) =>
    safeMutationCall(() => API.post("/auth/reset-password", data)),
  changePassword: (data) =>
    safeMutationCall(() => API.post("/auth/change-password", data)),
};
...
export const userApi = {
  ...
  getUserByEmployeeCode: (employeeCode) =>
    safeObjectCall(() =>
      API.get(`/users/employee-code/${encodeURIComponent(employeeCode)}`),
    ),
  getUsersByRole: (role) =>
    safeListCall(() => API.get(`/users/role/${encodeURIComponent(role)}`)),
  getUserById: (id) => safeObjectCall(() => API.get(`/users/${id}`)),
  createUser: (userData) =>
    safeMutationCall(() => API.post("/users", userData)),
  updateEmployee: (id, employeeData) =>
    safeMutationCall(() => API.put(`/users/employees/${id}`, employeeData)),
  updateUser: (id, userData) =>
    safeMutationCall(() => API.patch(`/users/${id}`, userData)),
  changeUserStatus: (id, status) =>
    safeMutationCall(() => API.patch(`/users/${id}/status`, { status })),
};

// --- 🏢 DEPARTMENT SERVICE ---
export const departmentApi = {
  createDepartment: (data) =>
    safeMutationCall(() => API.post("/departments", data)),
  getDepartments: (params) =>
    safeListCall(() => API.get("/departments", { params })),
  getDepartmentById: (id) =>
    safeObjectCall(() => API.get(`/departments/${id}`)),
  updateDepartment: (id, data) =>
    safeMutationCall(() => API.patch(`/departments/${id}`, data)),
};
...
export const deviceApi = {
  ...
  getDeviceCategoryById: (id) =>
    safeObjectCall(() => API.get(`/devices/categories/${id}`)),
  getDevicesByEmployee: (employeeId) =>
    safeListCall(() => API.get(`/devices/list/employees/${employeeId}`)),
  getDevicesByCategory: (categoryId) =>
    safeListCall(() => API.get(`/devices/list/categories/${categoryId}`)),
  getDeviceStateHistories: (id) =>
    safeListCall(() => API.get(`/devices/${id}/state-histories`)),
  assignDevice: (payload) =>
    safeMutationCall(() => API.post("/devices/assign-devices", payload)),
  getAssignRequests: (params) =>
    safeListCall(() => API.get("/devices/assign-requests", { params })),
  getAssignRequestsByEmployee: (employeeId, params) =>
    safeListCall(() =>
      API.get(`/devices/assign-requests/employees/${employeeId}`, { params }),
    ),
  getAssignRequestById: (id) =>
    safeObjectCall(() => API.get(`/devices/assign-requests/${id}`)),
  updateAssignRequest: (id, payload) =>
    safeMutationCall(() => API.put(`/devices/assign-requests/${id}`, payload)),
  createDevice: (deviceData) =>
    safeMutationCall(() => API.post("/devices", deviceData)),
  updateDevice: (id, deviceData) =>
    safeMutationCall(() => API.put(`/devices/${id}`, deviceData)),
...
export const maintenanceApi = {
  ...
  // --- MAINTENANCE PLANS ---
  getPlans: (params) =>
    safeListCall(() => API.get("/maintenances/plans", { params })),
  getPlanById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/plans/${id}`)),
  getPlansByStatus: (status, params) =>
    safeListCall(() =>
      API.get(`/maintenances/plans/status/${status}`, { params }),
    ),
  getPlanAssignments: (id, params) =>
    safeListCall(() =>
      API.get(`/maintenances/plans/assignments/${id}`, { params }),
    ),
  getPlanDocuments: (id, params) =>
    safeListCall(() =>
      API.get(`/maintenances/plans/${id}/documents`, { params }),
    ),
  createPlan: (data) =>
    safeMutationCall(() => API.post("/maintenances/plans", data)),
  updatePlan: (id, data) => API.put(`/maintenances/plans/${id}`, data),
  startPlan: (id, data) =>
    safeMutationCall(() => API.put(`/maintenances/plans/${id}/start`, data)),
  completePlan: (id, data) =>
    safeMutationCall(() => API.put(`/maintenances/plans/${id}/complete`, data)),

  // --- REPAIR REQUESTS ---
  getRepairs: (params) =>
    safeListCall(() => API.get("/maintenances/repairs", { params })),
  getRepairById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/repairs/${id}`)),
  createRepair: (data) =>
    safeMutationCall(() => API.post("/maintenances/repairs", data)),
  approveRepair: (id, data) =>
    safeMutationCall(() =>
      API.put(`/maintenances/repairs/${id}/approve`, data),
    ),

  // --- ADJUST PLAN REQUESTS ---
  createAdjustPlan: (data) =>
    safeMutationCall(() => API.post("/maintenances/adjust-plans", data)),
  getAdjustPlanById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/adjust-plans/${id}`)),
  approveAdjustPlan: (id, data) =>
    safeMutationCall(() =>
      API.put(`/maintenances/adjust-plans/${id}/approve`, data),
    ),

  // --- DAMAGE REPORTS ---
  createDamageReport: (data) =>
    safeMutationCall(() => API.post("/maintenances/damage-reports", data)),
  getDamageReportById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/damage-reports/${id}`)),

  // --- MAINTENANCE REQUESTS ---
  getMaintenanceRequests: (params) =>
    safeListCall(() =>
      API.get("/maintenances/maintenance-requests", { params }),
    ),
  createMaintenanceRequest: (data) =>
    safeMutationCall(() =>
      API.post("/maintenances/maintenance-requests", data),
    ),
  getMaintenanceRequestById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/maintenance-requests/${id}`)),
  approveMaintenanceRequest: (id, data) =>
    safeMutationCall(() =>
      API.put(`/maintenances/maintenance-requests/${id}/approve`, data),
    ),

  // --- ACCEPTANCE REPORTS ---
  getAcceptanceReports: (params) =>
    safeListCall(() => API.get("/maintenances/acceptance-reports", { params })),
  createAcceptanceReport: (data) =>
    safeMutationCall(() => API.post("/maintenances/acceptance-reports", data)),
  getAcceptanceReportById: (id) =>
    safeObjectCall(() => API.get(`/maintenances/acceptance-reports/${id}`)),
  approveAcceptanceReport: (id, data) =>
    safeMutationCall(() =>
      API.put(`/maintenances/acceptance-reports/${id}/approve`, data),
    ),

  getRequests: (params = {}) => {
    const status = String(params.status || "").toLowerCase();
    if (status === "completed")
      return maintenanceApi.getPlansByStatus("completed", params);
  ...
  approveCompletion: (id, data = {}) =>
    maintenanceApi.approveAcceptanceReport(id, {
      approved_by: data.approved_by,
      status:
        data.status || (data.decision === "APPROVED" ? "success" : "fail"),
    }),
...
export const inventoryApi = {
  ...
  getItemRequests: (params) =>
    safeListCall(() => API.get("/inventories/item-requests", { params })),
  getItemRequestById: (id) =>
    safeObjectCall(() => API.get(`/inventories/item-requests/${id}`)),
  getItemRequestsByPlanId: (planId) =>
    safeListCall(() => API.get(`/inventories/item-requests/plans/${planId}`)),
  createItemRequest: (data) =>
    safeMutationCall(() => API.post("/inventories/item-requests", data)),
  updateItemRequestStatus: (id, data) =>
    safeMutationCall(() => API.put(`/inventories/item-requests/${id}`, data)),
  ...
  getStockSummary: (params) =>
    safeListCall(() => API.get("/inventories", { params })),
};
...
export const notificationApi = {
  ...
  getEmployeeNotifications: (employeeId) =>
    safeListCall(() =>
      API.get(
        `/notifications/employees/${employeeId || getCurrentEmployeeId()}`,
        {
          params: { _t: Date.now() },
  ...
  markAllAsRead: async (employeeIdParam) => {
    ...
    const successfulIds = results
      .map((result, index) =>
        result.status === "fulfilled" ? unreadIds[index] : null,
      )
      .filter(Boolean);
  ...
  getUnreadCount: async () => {
    ...
    try {
      ...
      return Array.isArray(items)
        ? items.filter((item) => !item.is_read && !item.isRead).length
        : 0;
    } catch (error) {
...
export const reportApi = {
  getDashboardSummary: (params) =>
    safeObjectCall(() => API.get("/reports/dashboard", { params })),
  getDeviceReport: (params) =>
    safeListCall(() => API.get("/reports/devices", { params })),
  getMaintenanceReport: (params) =>
    safeListCall(() => API.get("/reports/maintenance", { params })),
  getTechnicianReport: (params) =>
    safeListCall(() => API.get("/reports/technicians", { params })),
  getCostReport: (params) =>
    safeListCall(() => API.get("/reports/costs", { params })),
  exportReport: (params) =>
    safeObjectCall(() => API.get("/reports/export", { params })),
};
